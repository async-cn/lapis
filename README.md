...
async def on_blockbreak(event: Event):
    ...
    if (
        ...
        await player.send_message(
            f"你正在使用 §e{get_item_name(tool)}§r 破坏 §e{get_item_name(block.block_id)}§r；\n"
...
FastShop 是一款仿制 QuickShop 插件的商店示例包，你可以点击[https://github.com/async-cn/fastshop](此处)前往获取。仅用于功能演示，请勿用于生产环境实装或倒卖收费等其他用途。
